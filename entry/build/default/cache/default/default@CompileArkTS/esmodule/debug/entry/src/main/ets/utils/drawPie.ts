...
import { percentageConversion, isPointInSector } from "@bundle:com.example.mccharts/entry/ets/utils/index";
class DrawPie extends Chart {
    ...
    clearGrid(index?) {
        let that = this, ctx = that.ctx, label = that.label, labelLine = that.labelLine, lineStyle = that.lineStyle, item, startAng = -Math.PI / 2, len = that.animateArr.filter(item => !item.hide).length, labels = [], j = 0, angle = 0;
        // 判断是否标签类型是在饼图外
        ...
        for (let i = 0, l = that.animateArr.length; i < l; i++) {
            ...
            if (label.show) {
                //画描述
                let tr = radius, textAlign = 'center', tw = ctx.measureText(item.name).width, th = ctx.measureText(item.name).height, tAng = startAng + item.ang / 2, x = tr * Math.cos(tAng), y = tr * Math.sin(tAng);
                if (isLine) {
                    ctx.lineWidth = width;
    ...
    animate() {
        let that = this, ctx = that.ctx, item, startAng, ang = 0, len = that.animateArr.filter(item => !item.hide).length, index = 0, i = 0, isStop = true;
        let [radius, innerRadius] = that.getRadius();
        isStop = true;
        // ctx.save();
