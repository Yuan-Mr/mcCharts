import { calculateNum, pointInsideCircle } from "@bundle:com.example.mccharts/entry/ets/utils/index";
import { Chart } from "@bundle:com.example.mccharts/entry/ets/utils/charts";
import { axisLineStyle, yLineStyle } from "@bundle:com.example.mccharts/entry/ets/utils/defaultOption";
/**
 * 散点图
 */
class DrawPoint extends Chart {
    ...
    bindEvent(e, callback) {
        let that = this, ctx = that.ctx, xl = this.xAxis.data.length, xs = (that.W - this.cPaddingL - this.cPaddingR) / xl, index = 0;
        let pos = {
            x: e.localX,
            y: e.localY
        };
    ...
    initData() {
        ...
        let that = this, xdis = this.W - this.cPaddingL - this.cPaddingR, ydis = this.H - this.cPaddingB - this.cPaddingT, xl = this.xAxis.data.length, xs = xdis / xl, xmin = Number(this.xAxis.data[0]), xmax = Number(this.xAxis.data.slice(-1)[0]), min, max, numMin, numMax, item, obj, arr = [], numArr = [], index = 0, r, h;
        if (!this.series.length)
            return;
        for (let i = 0; i < this.series.length; i++) {
            item = this.series[i];
    ...
    animate() {
        let that = this, ctx = this.ctx, start = new Date().getTime(), time = 0, item, obj, h, r, isStop = true;
        function run() {
            ctx.save();
    ...
    clearGrid(pos?) {
        let that = this, obj, item, ctx = this.ctx;
        ctx.clearRect(0, 0, that.W, that.H);
        // 画坐标系
        this.drawAxis();
        // // 画y轴刻度
