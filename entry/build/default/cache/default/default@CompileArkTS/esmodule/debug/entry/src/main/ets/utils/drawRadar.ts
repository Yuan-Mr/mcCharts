import { rotatePoint, isPointInsidePolygon, countDownLatch, percentageConversion, drawTexts, drawBreakText } from "@bundle:com.example.mccharts/entry/ets/utils/index";
import { Chart } from "@bundle:com.example.mccharts/entry/ets/utils/charts";
import { radar, itemStyle as commonItemStyle, areaStyle as commonAreaStyle, lineStyle as commonLineStyle } from "@bundle:com.example.mccharts/entry/ets/utils/defaultOption";
import type { InterfaceObj } from './chartInterface';
let points = [];
/**
 * 雷达图
 */
class DrawRadar extends Chart {
    ...
    bindEvent(e, callback) {
        ...
        let { indicator = [], center = radar.center, radius = radar.radius, startAngle: radarStartAngle = radar.startAngle, splitNumber = radar.splitNumber, splitLine = {}, splitArea = {}, } = this.radar;
    ...
    drawRadarBg() {
        const ctx = this.ctx;
        const w = this.W;
        let { indicator = [], center = radar.center, radius = radar.radius, startAngle: radarStartAngle = radar.startAngle, splitNumber = radar.splitNumber, axisLine = {}, splitLine = {}, splitArea = {}, } = this.radar;
        // 雷达图分割线
    ...
    drawText() {
        const ctx = this.ctx;
        const w = this.W;
        let { indicator = [], radius = radar.radius, center = radar.center, nameGap = radar.nameGap, axisName = {}, startAngle: radarStartAngle = radar.startAngle } = this.radar;
        const { show = true, color = '#999', fontWeight = 'normal', fontFamily = 'sans-serif', fontSize = 22, width = 0, overflow = 'none' } = axisName;
        if (!show)
            return;
    ...
    async animate(activeIndex = null) {
        ...
        let ifTip = false;
        let tipArr = null;
        let { indicator = [], center = radar.center, radius = radar.radius, startAngle: radarStartAngle = radar.startAngle } = this.radar;
        this.drawing = true;
        // 雷达图半径
        radius = typeof radius === 'string' && radius.indexOf('%') !== -1 ? (w / 2) * percentageConversion(radius) : (Number(radius) || (w / 2 * 0.5));
        // 移动到图表中间
